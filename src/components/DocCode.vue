<template>
  <div class="doc-code" :class="{ 'doc-code--collapsed': needsCollapse && collapsed && !expanded }">
    ...
    <div ref="bodyRef" class="doc-code__body" :style="bodyStyle" v-html="wrappedHtml"></div>
  </div>
</template>
...
const highlighted = computed(() => {
  if (highlightResult.value) return highlightResult.value.value
  // 兜底：纯文本（HTML 转义）
  return props.code
    .replace(/&/g, '&amp;')
    .replace(/</g, '&lt;')
...
const effectiveLang = computed(() => {
  if (props.lang && props.lang !== 'auto') return props.lang
...
const bodyStyle = computed(() => {
  if (!needsCollapse.value || expanded.value) return {}
