  ...
  <div style="background-color: #e5e7eb">
    <el-header :style="headerStyle">
      <div style="padding: 5px 0; text-align: right">
        ...
        <el-button
          size="small"
          v-if="'CLASSICS' === logicJson.modelValue"
          icon="Rank"
          @click="zoomViewport(1)"
          >自适应
        </el-button>
        ...
        <el-button size="small" type="primary" @click="saveJsonModel">保存</el-button>
      </div>
...
import PropertySetting from '@/components/xly-flow-designer/common/vue/propertySetting.vue'
import {
  getPreviousNodes,
  isClassics,
  json2LogicFlowJson,
  logicFlowJsonToWarmFlow,
} from '@/components/xly-flow-designer/common/js/tool.js'
import StartC from '@/components/xly-flow-designer/classics/js/start.js'
import BetweenC from '@/components/xly-flow-designer/classics/js/between.js'
import SerialC from '@/components/xly-flow-designer/classics/js/serial.js'
import ParallelC from '@/components/xly-flow-designer/classics/js/parallel.js'
import EndC from '@/components/xly-flow-designer/classics/js/end.js'
import SkipC from '@/components/xly-flow-designer/classics/js/skip.js'
import StartM from '@/components/xly-flow-designer/mimic/js/start.js'
import BetweenM from '@/components/xly-flow-designer/mimic/js/between.js'
import SerialM from '@/components/xly-flow-designer/mimic/js/serial.js'
import ParallelM from '@/components/xly-flow-designer/mimic/js/parallel.js'
import EndM from '@/components/xly-flow-designer/mimic/js/end.js'
import SkipM from '@/components/xly-flow-designer/mimic/js/skip.js'
import { computed, onMounted, onUnmounted, ref, watch } from 'vue'
import initClassicsData from '@/components/xly-flow-designer/classics/initClassicsData.json'
import initMimicData from '@/components/xly-flow-designer/mimic/initMimicData.json'
import {
  addBetweenNode,
  addGatewayNode,
  gatewayAddNode,
  removeNode,
} from '@/components/xly-flow-designer/mimic/js/mimic.js'
import EdgeTooltip from '@/components/xly-flow-designer/mimic/vue/EdgeTooltip.vue'
...
watch(
  ...
  (newVal, oldVal) => {
    if (newVal) {
      ...
      propertySettingRef.value = []
      logicJson.value = {}
      jsonString.value = ''
      skipConditionShow.value = true
      nodes.value = []
      skips.value = []
      isDark.value = false
      activeStep.value = 1
      onlyDesignShow.value = true
      defFlowId.value = null
      dataJson.value = ''
      initFlow(newVal)
    }
  },
...
function initFlow(flow: any) {
  if (flow) {
    ...
    jsonString.value = dataJson.value
    if (jsonString.value) {
      ...
      console.log('logicJson.value', logicJson.value)
...
function initEvent() {
  ...
  eventCenter.on('edge:add', (args) => {
    ...
    const previousNodes = getPreviousNodes(
      graphData['nodes'],
      graphData['edges'],
      args.data.sourceNodeId,
    )
    lf.value.changeEdgeType(args.data.id, 'skip')
    // 修改边类型
    lf.value.setProperties(args.data.id, {
      skipType: previousNodes.some((node) => node.id === args.data.targetNodeId)
        ? 'REJECT'
        : 'PASS',
    })
  })
