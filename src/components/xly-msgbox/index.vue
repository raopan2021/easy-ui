      ...
      >
        <Transition name="xly-msgbox-zoom">
          ...
          >
            ...
            <!-- 头部：图标 + 标题 -->
            <div
              class="xly-msgbox__header"
              :class="{ 'xly-msgbox__header--no-title': !state.options.title }"
            >
              <div
                v-if="showIcon"
              ...
              <span v-if="state.options.title" class="xly-msgbox__title">{{
                state.options.title
              }}</span>
            </div>
            ...
            <div class="xly-msgbox__body">
              ...
              <!-- 输入框 -->
              <div
                v-if="state.options.showInput"
                class="xly-msgbox__input-wrap"
              >
                <textarea
                ...
                <Transition name="xly-msgbox-err">
                  <span
                    ...
                    class="xly-msgbox__input-error"
                    >{{ state.inputError }}</span
                  >
                </Transition>
              </div>
            </div>
            ...
            <div class="xly-msgbox__footer">
              <button
                v-if="
                  state.options.showCancelButton !== false &&
  ...
  if (state.options.showInput) {
    ...
    if (pattern) {
      ...
      if (!reg.test(state.inputValue)) {
        state.inputError =
          state.options.input?.patternMessage || "输入内容不符合要求";
        return;
      }
    }
    if (
      !state.inputValue.trim() &&
      state.options.input?.inputType !== "number"
    ) {
...
.xly-msgbox__btn {
  ...
  &--cancel {
    ...
    &:hover:not(:disabled) {
      background: #ebedf5;
      border-color: color.adjust(#e4e6f0, $lightness: -6%);
    }
