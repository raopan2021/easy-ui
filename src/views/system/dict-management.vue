<template>
  <div class="system-page">
    <div class="page-header">
      ...
      <p class="page-header__desc">管理系统字典类型及字典数据，左侧为字典类型，右侧为对应类型的字典数据</p>
    </div>
    ...
    >
      ...
      <template #footer>
        ...
        <el-button type="primary" :loading="dataDialog.loading" @click="handleDataSubmit">确定</el-button>
      </template>
    </el-dialog>
  </div>
</template>
...
function getInitTypeForm() { return { id: 0, name: '', code: '', status: 1 } }
function handleAddType() { typeDialog.isEdit = false; typeDialog.form = getInitTypeForm(); typeDialog.visible = true }
function handleEditType(row: DictTypeItem) { typeDialog.isEdit = true; typeDialog.form = { ...row }; typeDialog.visible = true }
function handleTypeDialogClose() { typeFormRef.value?.resetFields(); typeDialog.form = getInitTypeForm() }
    ...
    if (typeDialog.isEdit) {
      ...
      if (idx !== -1) {
        ...
        if (oldCode !== typeDialog.form.code) {
          allDataItems.forEach(d => { if (d.typeCode === oldCode) d.typeCode = typeDialog.form.code })
        }
        // 更新选中项
...
function fetchDataData() {
  ...
  setTimeout(() => {
    ...
    dataTableData.value = filtered.slice((dataPagination.page - 1) * dataPagination.pageSize, dataPagination.page * dataPagination.pageSize)
...
function getInitDataForm() { return { id: 0, typeCode: '', label: '', value: '', tagType: 'primary', sort: 0, status: 1 } }
...
function handleEditData(row: DictDataItem) { dataDialog.isEdit = true; dataDialog.form = { ...row }; dataDialog.visible = true }
function handleDataDialogClose() { dataFormRef.value?.resetFields(); dataDialog.form = getInitDataForm() }
...
async function handleDataSubmit() {
  ...
  setTimeout(() => {
    if (dataDialog.isEdit) {
      ...
      if (idx !== -1) allDataItems[idx] = { ...allDataItems[idx], ...dataDialog.form }
      ElMessage.success('编辑成功')
    } else {
...
onMounted(() => {
  ...
  // 默认选中第一项
  setTimeout(() => {
    if (typeData.value.length) handleTypeSelect(typeData.value[0])
  }, 200)
})
</script>
