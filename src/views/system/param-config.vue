<template>
  <div class="system-page">
    <div class="page-header">
      ...
      <p class="page-header__desc">管理系统的全局参数配置，支持文本、数字、布尔、JSON 等多种类型</p>
    </div>
      ...
      <el-table-column prop="value" label="参数值" min-width="180">
        <template #default="{ row }">
          <template v-if="row.type === 'boolean'">
            <el-tag :type="row.value === 'true' ? 'success' : 'danger'" size="small">{{ row.value === 'true' ? '是' : '否' }}</el-tag>
          </template>
...
function genMockData(): ParamItem[] {
  return [
    { id: 1, key: 'upload.max_size', value: '50', type: 'number', remark: '文件上传大小限制(MB)', status: 1, createTime: '2026-01-10 09:00:00' },
    { id: 2, key: 'upload.allowed_types', value: 'jpg,png,gif,pdf,docx,xlsx', type: 'text', remark: '允许上传的文件类型', status: 1, createTime: '2026-01-10 09:30:00' },
    { id: 3, key: 'system.enable_register', value: 'false', type: 'boolean', remark: '是否开放注册', status: 1, createTime: '2026-01-15 10:00:00' },
    { id: 4, key: 'system.session_timeout', value: '1800', type: 'number', remark: '会话超时时间(秒)', status: 1, createTime: '2026-01-15 10:30:00' },
    { id: 5, key: 'mail.smtp_config', value: '{"host":"smtp.example.com","port":465,"ssl":true}', type: 'json', remark: '邮件服务器配置', status: 1, createTime: '2026-02-20 14:00:00' },
    { id: 6, key: 'system.maintenance_mode', value: 'false', type: 'boolean', remark: '维护模式开关', status: 0, createTime: '2026-03-01 08:00:00' },
    { id: 7, key: 'security.password_min_length', value: '8', type: 'number', remark: '密码最小长度', status: 1, createTime: '2026-03-15 11:00:00' },
    { id: 8, key: 'security.login_retry_limit', value: '5', type: 'number', remark: '登录重试次数上限', status: 1, createTime: '2026-04-10 09:00:00' },
  ]
}
...
function handleReset() { searchForm.keyword = ''; searchForm.type = ''; searchForm.status = null; pagination.page = 1; fetchData() }
...
function handleSelectionChange(rows: ParamItem[]) { selectedIds.value = rows.map(r => r.id) }
...
function fetchData() {
  ...
  setTimeout(() => {
    ...
    if (searchForm.type) filtered = filtered.filter(r => r.type === searchForm.type)
...
function onTypeChange() {
  if (dialog.form.type === 'number') dialog.form.value = String(dialog.form.numberValue)
  else if (dialog.form.type === 'boolean') dialog.form.value = String(dialog.form.boolValue)
...
function handleAdd() { dialog.isEdit = false; Object.assign(dialog.form, getInitForm()); dialog.visible = true }
...
function handleEdit(row: ParamItem) {
  ...
  if (row.type === 'number') { dialog.form.value = row.value; dialog.form.numberValue = Number(row.value) }
  else if (row.type === 'boolean') { dialog.form.value = row.value; dialog.form.boolValue = row.value === 'true' }
  else dialog.form.value = row.value
  dialog.visible = true
}

function handleDialogClose() { formRef.value?.resetFields(); Object.assign(dialog.form, getInitForm()) }
...
async function handleSubmit() {
  ...
  setTimeout(() => {
    ...
    if (dialog.form.type === 'number') valueStr = String(dialog.form.numberValue)
    else if (dialog.form.type === 'boolean') valueStr = String(dialog.form.boolValue)
