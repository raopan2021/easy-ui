<template>
  <div class="system-page">
    <div class="page-header">
      <h2 class="page-header__title">功能权限</h2>
      <p class="page-header__desc">管理系统的功能权限点，支持模块和权限点的两级树形结构</p>
    </div>
    ...
    >
      ...
      <el-table-column prop="name" label="名称" min-width="180">
        <template #default="{ row }">
          ...
          <el-tag v-if="row.type === 'module'" size="small" type="info" style="margin-left: 8px;">模块</el-tag>
        </template>
      </el-table-column>
...
import { Plus, Delete, Search, RefreshRight, FolderAdd } from '@element-plus/icons-vue'
...
function genMockData(): PermissionNode[] {
  return [
    { id: 1, name: '系统管理', code: 'system', type: 'module', parentId: 0, sort: 1, status: 1, createTime: '2026-01-15 10:00:00', children: [
      { id: 11, name: '用户管理-查看', code: 'system:user:list', type: 'menu', parentId: 1, sort: 1, status: 1, createTime: '2026-01-15 10:00:00' },
      { id: 12, name: '用户管理-新增', code: 'system:user:add', type: 'button', parentId: 1, sort: 2, status: 1, createTime: '2026-01-15 10:00:00' },
      { id: 13, name: '用户管理-编辑', code: 'system:user:edit', type: 'button', parentId: 1, sort: 3, status: 1, createTime: '2026-01-15 10:00:00' },
      { id: 14, name: '用户管理-删除', code: 'system:user:delete', type: 'button', parentId: 1, sort: 4, status: 1, createTime: '2026-01-15 10:00:00' },
    ]},
    { id: 2, name: '内容管理', code: 'content', type: 'module', parentId: 0, sort: 2, status: 1, createTime: '2026-01-15 10:00:00', children: [
      { id: 21, name: '文章列表', code: 'content:article:list', type: 'menu', parentId: 2, sort: 1, status: 1, createTime: '2026-01-15 10:00:00' },
      { id: 22, name: '文章管理', code: 'content:article:manage', type: 'button', parentId: 2, sort: 2, status: 1, createTime: '2026-01-15 10:00:00' },
    ]},
    { id: 3, name: '数据分析', code: 'analytics', type: 'module', parentId: 0, sort: 3, status: 0, createTime: '2026-01-15 10:00:00', children: [
      { id: 31, name: '数据导出', code: 'analytics:export', type: 'api', parentId: 3, sort: 1, status: 1, createTime: '2026-01-15 10:00:00' },
    ]},
    { id: 4, name: '设置中心', code: 'settings', type: 'module', parentId: 0, sort: 4, status: 1, createTime: '2026-01-15 10:00:00', children: [] },
  ]
}
...
function handleReset() { searchForm.keyword = ''; searchForm.type = ''; fetchData() }

function handleSelectionChange(rows: PermissionNode[]) { selectedRows.value = rows }
...
const dialogTitle = computed(() => {
  if (dialog.isEdit) return dialog.form.type === 'module' ? '编辑模块' : '编辑权限点'
  if (dialog.form.type === 'module') return '新增模块'
  return '新增权限点'
})
...
function handleEdit(row: PermissionNode) {
  dialog.isEdit = true
  dialog.parentNode = null
  dialog.form = { id: row.id, name: row.name, code: row.code, type: row.type, parentId: row.parentId, sort: row.sort, status: row.status }
  dialog.visible = true
}
...
function findAndReplace(list: PermissionNode[], id: number, replacer: (node: PermissionNode) => PermissionNode | null): PermissionNode[] {
  return list.reduce<PermissionNode[]>((acc, node) => {
    if (node.id === id) {
      const replaced = replacer(node)
      if (replaced) acc.push(replaced)
    } else {
...
function findById(list: PermissionNode[], id: number): PermissionNode | null {
  for (const n of list) {
    if (n.id === id) return n
...
function gatherIds(list: PermissionNode[]): number[] {
  return list.reduce<number[]>((acc, n) => {
    acc.push(n.id)
    if (n.children?.length) acc.push(...gatherIds(n.children))
    return acc
  }, [])
}
