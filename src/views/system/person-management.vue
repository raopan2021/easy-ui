<template>
  <div class="system-page">
    <div class="page-header">
      ...
      <p class="page-header__desc">管理机构人员信息，支持按部门、岗位筛选</p>
    </div>
    ...
    <div class="search-bar">
      <el-input
        ...
        @clear="handleSearch"
      />
      <el-select v-model="searchForm.deptId" placeholder="所属部门" clearable style="width: 160px" @change="onDeptFilterChange">
    ...
    >
      <el-form ref="formRef" :model="dialog.form" :rules="formRules" label-width="90px">
        ...
        <el-form-item label="状态" prop="status">
          <el-switch v-model="dialog.form.status" :active-value="1" :inactive-value="0" active-text="在职" inactive-text="离职" />
        </el-form-item>
      </el-form>
      <template #footer>
...
const deptOptions = [
  { id: 1, name: '技术部' }, { id: 11, name: '前端组' }, { id: 12, name: '后端组' },
  { id: 2, name: '产品部' }, { id: 3, name: '市场部' }, { id: 4, name: '销售部' },
]

// 岗位按部门分组
const postsByDept: Record<number, { id: number; name: string }[]> = {
  1: [{ id: 7, name: '技术总监' }],
  11: [{ id: 1, name: '高级前端工程师' }, { id: 2, name: '中级前端工程师' }],
...
function genMockData(): PersonItem[] {
  return [
    { id: 1, name: '张伟', empNo: 'EMP001', deptId: 11, deptName: '前端组', postId: 1, postName: '高级前端工程师', phone: '13800138001', status: 1, createTime: '2025-03-01 09:00:00' },
    { id: 2, name: '李娜', empNo: 'EMP002', deptId: 11, deptName: '前端组', postId: 2, postName: '中级前端工程师', phone: '13800138002', status: 1, createTime: '2025-04-15 09:00:00' },
    { id: 3, name: '王强', empNo: 'EMP003', deptId: 12, deptName: '后端组', postId: 3, postName: 'Java开发工程师', phone: '13800138003', status: 1, createTime: '2025-05-10 09:00:00' },
    { id: 4, name: '赵敏', empNo: 'EMP004', deptId: 2, deptName: '产品部', postId: 4, postName: '产品经理', phone: '13800138004', status: 1, createTime: '2025-06-01 09:00:00' },
    { id: 5, name: '孙磊', empNo: 'EMP005', deptId: 4, deptName: '销售部', postId: 6, postName: '销售代表', phone: '13800138005', status: 1, createTime: '2025-07-15 09:00:00' },
    { id: 6, name: '周婷', empNo: 'EMP006', deptId: 3, deptName: '市场部', postId: 5, postName: '市场专员', phone: '13800138006', status: 0, createTime: '2025-08-01 09:00:00' },
  ]
}
...
// --- search ---
const searchForm = reactive({ keyword: '', deptId: null as number | null, postId: null as number | null, status: null as number | null })
const filteredPosts = computed(() => {
  if (searchForm.deptId) return postsByDept[searchForm.deptId] || []
  return Object.values(postsByDept).flat()
})
function onDeptFilterChange() { searchForm.postId = null; handleSearch() }
function handleSearch() { pagination.page = 1; fetchData() }
function handleReset() { searchForm.keyword = ''; searchForm.deptId = null; searchForm.postId = null; searchForm.status = null; pagination.page = 1; fetchData() }
...
function handleSelectionChange(rows: PersonItem[]) { selectedIds.value = rows.map(r => r.id) }
...
function fetchData() {
  ...
  setTimeout(() => {
    ...
    if (searchForm.deptId) filtered = filtered.filter(r => r.deptId === searchForm.deptId)
    if (searchForm.postId) filtered = filtered.filter(r => r.postId === searchForm.postId)
...
const dialog = reactive({
  ...
  form: {} as { id: number; name: string; empNo: string; deptId: number; deptName: string; postId: number; postName: string; phone: string; status: number },
})

const dialogPosts = computed(() => {
  if (dialog.form.deptId) return postsByDept[dialog.form.deptId] || []
  return Object.values(postsByDept).flat()
})
...
function getInitForm() { return { id: 0, name: '', empNo: '', deptId: 0, deptName: '', postId: 0, postName: '', phone: '', status: 1 } }
function handleAdd() { dialog.isEdit = false; dialog.form = getInitForm(); dialog.visible = true }
function handleEdit(row: PersonItem) { dialog.isEdit = true; dialog.form = { ...row }; dialog.visible = true }
...
async function handleSubmit() {
  ...
  setTimeout(() => {
    ...
    const post = Object.values(postsByDept).flat().find(p => p.id === dialog.form.postId)
...
async function handleBatchDelete() {
  try { await ElMessageBox.confirm(`确定删除选中的 ${selectedIds.value.length} 条？`, '批量删除', { type: 'warning' }) } catch { return }
