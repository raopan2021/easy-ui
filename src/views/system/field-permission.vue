<template>
  <div class="system-page">
    <div class="page-header">
      <h2 class="page-header__title">字段权限</h2>
      <p class="page-header__desc">配置字段级别的访问控制，控制各角色对数据表字段的可见、编辑、隐藏权限</p>
    </div>
    ...
    <div class="search-bar">
      <el-input
        ...
        @clear="handleSearch"
      />
      <el-select v-model="searchForm.tableName" placeholder="数据表" clearable style="width: 180px" @change="handleSearch">
        <el-option v-for="t in tableOptions" :key="t" :label="t" :value="t" />
      </el-select>
      <el-select v-model="searchForm.permType" placeholder="权限类型" clearable style="width: 130px" @change="handleSearch">
    ...
    >
      ...
      <el-table-column prop="fieldLabel" label="字段说明" min-width="140" />
      <el-table-column prop="permType" label="权限类型" width="110">
        <template #default="{ row }">
          <el-tag :type="permTypeMap[row.permType]?.tag" size="small">
    ...
    >
      <el-form ref="formRef" :model="dialog.form" :rules="formRules" label-width="90px">
        ...
        <el-form-item label="字段名" prop="fieldName">
          <el-input v-model="dialog.form.fieldName" placeholder="如: phone_number" maxlength="50" />
        </el-form-item>
        <el-form-item label="字段说明" prop="fieldLabel">
          <el-input v-model="dialog.form.fieldLabel" placeholder="如: 手机号码" maxlength="30" />
        </el-form-item>
        <el-form-item label="权限类型" prop="permType">
          <el-radio-group v-model="dialog.form.permType">
...
function genMockData(): FieldRuleItem[] {
  return [
    { id: 1, tableName: 'sys_user', fieldName: 'password', fieldLabel: '密码', permType: 'hidden', status: 1, createTime: '2026-01-15 09:00:00' },
    { id: 2, tableName: 'sys_user', fieldName: 'phone', fieldLabel: '手机号', permType: 'visible', status: 1, createTime: '2026-01-15 09:30:00' },
    { id: 3, tableName: 'sys_user', fieldName: 'email', fieldLabel: '邮箱', permType: 'editable', status: 1, createTime: '2026-01-15 10:00:00' },
    { id: 4, tableName: 'sys_user', fieldName: 'salary', fieldLabel: '薪资', permType: 'hidden', status: 1, createTime: '2026-01-20 14:00:00' },
    { id: 5, tableName: 'biz_order', fieldName: 'total_amount', fieldLabel: '订单总额', permType: 'visible', status: 1, createTime: '2026-02-10 11:00:00' },
    { id: 6, tableName: 'biz_order', fieldName: 'cost_price', fieldLabel: '成本价', permType: 'hidden', status: 1, createTime: '2026-02-10 11:30:00' },
    { id: 7, tableName: 'biz_customer', fieldName: 'id_card', fieldLabel: '身份证号', permType: 'hidden', status: 1, createTime: '2026-03-05 15:00:00' },
    { id: 8, tableName: 'biz_customer', fieldName: 'contact_name', fieldLabel: '联系人', permType: 'editable', status: 0, createTime: '2026-03-05 15:30:00' },
  ]
}
...
function handleReset() { searchForm.keyword = ''; searchForm.tableName = ''; searchForm.permType = ''; pagination.page = 1; fetchData() }
...
function handleSelectionChange(rows: FieldRuleItem[]) { selectedIds.value = rows.map(r => r.id) }
...
function fetchData() {
  ...
  setTimeout(() => {
    ...
    if (searchForm.tableName) filtered = filtered.filter(r => r.tableName === searchForm.tableName)
    if (searchForm.permType) filtered = filtered.filter(r => r.permType === searchForm.permType)
...
const dialog = reactive({
  ...
  isEdit: false,
  loading: false,
  form: {} as { id: number; tableName: string; fieldName: string; fieldLabel: string; permType: string; status: number },
})
...
function getInitForm() { return { id: 0, tableName: '', fieldName: '', fieldLabel: '', permType: 'visible', status: 1 } }

function handleAdd() { dialog.isEdit = false; dialog.form = getInitForm(); dialog.visible = true }
function handleEdit(row: FieldRuleItem) { dialog.isEdit = true; dialog.form = { ...row }; dialog.visible = true }
...
async function handleSubmit() {
  ...
  setTimeout(() => {
    if (dialog.isEdit) {
      ...
      if (idx !== -1) allData[idx] = { ...allData[idx], ...dialog.form } as FieldRuleItem
