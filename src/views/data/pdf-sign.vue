<template>
  <div class="pdf-sign-doc">
    <div class="doc-header">
      <h1 class="doc-title">PdfSign PDF 签名</h1>
      <p class="doc-desc">
              ...
              <div class="pdf-page">
                ...
                <div class="pdf-page__body">
                  ...
                  <div class="signature-area">
                    ...
                    >
                      ...
                      <canvas
                        v-else
                        ref="sigCanvasRefs"
                        class="signature-canvas-small"
                        :width="160"
                        :height="60"
                      />
                      <el-button
                        class="signature-item__del"
                        size="small"
                        circle
                        @click.stop="removeSig(idx)"
                        link
                      >
                  ...
                  <div class="signature-area" style="margin-left: 240px">
                    <p style="font-size: 13px; color: #999">拖拽签名到此处</p>
                  </div>
                  <p>&nbsp;</p>
                  <p style="text-align: right; margin-top: 40px">签订日期：2026 年 7 月 27 日</p>
                </div>
              </div>
            </div>
              ...
              <el-tabs model-value="handwrite">
                <el-tab-pane label="手写签名" name="handwrite">
                  ...
                  <div class="draw-actions">
                    ...
                    <el-button size="small" type="primary" @click="addSignature">添加签名</el-button>
                  </div>
                </el-tab-pane>
    ...
    <section class="doc-section">
      ...
      <div class="doc-table">
        <table>
          ...
          <tbody>
            <tr><td>pdf-url</td><td>PDF 文件地址</td><td><code>string</code></td><td><code>—</code></td></tr>
            <tr><td>signatures</td><td>已添加的签名列表</td><td><code>Signature[]</code></td><td><code>[]</code></td></tr>
            <tr><td>readonly</td><td>是否只读模式</td><td><code>boolean</code></td><td><code>false</code></td></tr>
            <tr><td>pen-color</td><td>签名笔颜色</td><td><code>string</code></td><td><code>#1a1a2e</code></td></tr>
          </tbody>
        </table>
      </div>
      ...
      <div class="doc-table">
        <table>
          ...
          <tbody>
            <tr><td>add</td><td>添加签名时触发</td><td><code>(signature: Signature)</code></td></tr>
            <tr><td>remove</td><td>删除签名时触发</td><td><code>(index: number)</code></td></tr>
            <tr><td>move</td><td>移动签名时触发</td><td><code>(index, position)</code></td></tr>
          </tbody>
        </table>
      </div>
    </section>
...
function renderSigCanvases() {
  nextTick(() => {
    ...
    canvases.forEach((c, idx) => {
      ...
      if (!sig || sig.type !== 'handwrite') return
      const img = new Image()
      img.onload = () => {
...
.pdf-page {
  ...
  &__header {
    text-align: center;
    margin-bottom: 30px;
    h3 { margin: 0 0 4px; font-size: 14px; color: #64748b; font-weight: 400; }
    h4 { margin: 0; font-size: 22px; color: #1a1a2e; }
  }
  &__body {
    p {
      font-size: 14px;
