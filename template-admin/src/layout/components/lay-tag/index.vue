...
import { ArrowDown, ArrowLeft as ArrowLeftSLine, ArrowRight as ArrowRightSLine } from '@element-plus/icons-vue'
import {
  delay,
  isAllEmpty,
  isEqual,
...
const ExitFullscreen = makeIcon('exit_screen')
...
const { bool: isShowArrow, setBool: setShowArrow } = useBoolean()
...
async function moveToView(index: number): Promise<void> {
  ...
  setShowArrow(scrollbarDomWidth <= tabDomWidth)
    ...
    case 6:
      ...
      setTimeout(() => {
        if (pureSetting.hiddenSideBar) {
          tagsViews[6].icon = markRaw(ExitFullscreen)
          tagsViews[6].text = '内容区退出全屏'
        }
        else {
          tagsViews[6].icon = markRaw(Fullscreen)
...
<template>
  <div v-if="!showTags" ref="containerDom" class="tags-view">
    ...
    <span v-show="isShowArrow" class="arrow-right">
      <PureIcon :icon="ArrowRightSLine" :size="20" @click="handleScroll(-200)" />
    </span>
    <!-- 右键菜单按钮 -->
