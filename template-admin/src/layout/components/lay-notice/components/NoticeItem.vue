...
const { bool: titleTooltip, setBool: setTitleTooltip } = useBoolean()
const descriptionRef = ref(null)
const { bool: descriptionTooltip, setBool: setDescriptionTooltip } = useBoolean()
const { tooltipEffect } = useNav()
const isMobile = deviceDetection()

function hoverTitle() {
  nextTick(() => {
    setTitleTooltip(titleRef.value?.scrollWidth > titleRef.value?.clientWidth)
  })
}
...
function hoverDescription(event, description) {
  ...
  setDescriptionTooltip(currentWidth > 2 * cellWidth)
}
</script>
