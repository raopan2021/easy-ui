...
import type { GutterInjection } from '../xly-row/types'
import { computed, inject, ref } from 'vue'
import { ROW_GUTTER_KEY } from '../xly-row/types'
...
const colClasses = computed(() => {
  ...
  if (props.offset)
    classes.push(`xly-col--offset-${props.offset}`)
  if (props.push)
    classes.push(`xly-col--push-${props.push}`)
  if (props.pull)
    classes.push(`xly-col--pull-${props.pull}`)
  ...
  for (const bp of breakpoints) {
    ...
    if (typeof val === 'number') {
      ...
      if (val.span)
        classes.push(`xly-col--${bp}-${val.span}`)
      if (val.offset)
        classes.push(`xly-col--${bp}-offset-${val.offset}`)
      if (val.push)
        classes.push(`xly-col--${bp}-push-${val.push}`)
      if (val.pull)
        classes.push(`xly-col--${bp}-pull-${val.pull}`)
    }
  }
