 ...
 * @returns
 */
export function logicFlowJsonToWarmFlow(data) {
  // 先构建成流程对象
  const definition = {
    nodeList: [],
  }
  ...
  data.nodes.forEach((anyNode) => {
    ...
    for (const key in anyNode.properties.ext) {
      if (Object.hasOwn(anyNode.properties.ext, key)) {
...
export function setCommonStyle(style, properties, nodeType, type) {
  ...
  const [doneColor, todoColor, notDoneColor] = ['22, 119, 255', '250, 173, 20', '156, 163, 175']
  const isDark = document.documentElement.classList.contains('dark')

  // mimic 类型由 Vue 组件自行绘制边框，LogicFlow 的 SVG stroke 设为透明
  if (type === 'mimic') {
  ...
  switch (properties.status) {
    ...
    default:
      // 默认状态 — 中性灰
      if (isNode) {
        style.fill = isDark ? 'rgba(0, 0, 0, 0.2)' : 'rgba(255, 255, 255)'
        style.stroke = isDark ? 'rgb(180, 180, 190)' : `rgb(${notDoneColor})`
      }
      break
  }
...
export function getPreviousNodes(nodes, skips, nowNodeCode) {
  const previousCode = getPreviousCode(skips, nowNodeCode, new Set())
  // 使用 Set 去重后再转换为数组
...
function getPreviousCode(skips, nowNodeCode, visited = new Set()) {
  ...
  const passSkip = skips.filter(skip => skip.properties.skipType === 'PASS')
  const previousCode = []
