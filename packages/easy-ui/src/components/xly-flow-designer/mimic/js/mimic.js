...
function deleteAndAddEdge(
  ...
  if (nextEdges.length > 1) {
    for (let i = 0; i < nextEdges.length; i++) {
      ...
      // 边重复检测
      if (
        ['serial', 'parallel'].includes(sourceNode.type)
        && edgeMap.has(`${edge.sourceNodeId}:${edge.targetNodeId}`)
      ) {
        continue // 跳过重复边
      }
 ...
 * 水平自动布局，不需要移动的节点
 */
function LevelNotMoveNode(targetEdges, nodes, edges, lf, visitedIds, type = true, nodeMap = new Map()) {
  if (nodeMap.size === 0) {
...
export function addBetweenNode(lf, edge) {
  const nodes = lf.getGraphData().nodes
...
export function addGatewayNode(lf, edge, nodeType) {
  const nodes = lf.getGraphData().nodes
  const edges = lf.getGraphData().edges
  const sourceNode = nodes.find(node => node.id === edge.sourceNodeId)
  const targetNode = nodes.find(node => node.id === edge.targetNodeId)
...
export function gatewayAddNode(lf, gatewayNode) {
  ...
  // 连接互斥网关开始节点到新的中间节点
  const betweenNode = addNode(lf, 'between', mostChild.x + OFFSET_X * 2, getMoveY(gatewayNode, 'between'), '中间节点')
...
export function removeNode(lf, nodeModel) {
  ...
  if (
    ['serial', 'parallel'].includes(sourceNode.type)
    && ['serial', 'parallel'].includes(targetNode.type)
    ...
    if (lf.getNodeOutgoingNode(sourceNode.id).length >= 2) {
      ...
      const mostLeftX = sourceNode.x - (xSet.size - 1) * OFFSET_X
      let moveX
      regionNodes.forEach((node) => {
        if (gatewayEnd.id !== node.id) {
          if (moveX) {
    ...
    moveLevelNode(
      ...
      sourceEdges,
      lf,
      nodeModel.x < sourceNode.x ? getLeftmostNode(visitedNodes) : null,
      nodeModel.x < sourceNode.x ? null : getRightmostNode(visitedNodes),
      false,
      visitedIds,
    )
  }
...
function moveLevelNode(
  ...
  const needMoveLeftNode = []
  const needMoveRightNode = []
  ...
  const n = betweenLeft && betweenRight ? 1 : 2
  // 水平自动布局
  if (LeftMove) {
