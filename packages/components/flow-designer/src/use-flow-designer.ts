...
// 流程设计器组合式逻辑（从 flow-designer.vue 抽离，保持原行为一致）
...
import ParallelC from './classics/js/parallel'
import SerialC from './classics/js/serial'
...
import EndM from './mimic/js/end'
import { addBetweenNode, addGatewayNode, gatewayAddNode, removeNode } from './mimic/js/mimic'
import ParallelM from './mimic/js/parallel'
import SerialM from './mimic/js/serial'
import SkipM from './mimic/js/skip'
import StartM from './mimic/js/start'

export function useFlowDesigner(props: FlowDesignerProps, emit: FlowDesignerEmits) {
    ...
    if (flow) {
      ...
      if (jsonString.value) {
        if (!logicJson.value.nodes || logicJson.value.nodes.length === 0) {
          const initData = isClassics(logicJson.value.modelValue) ? initClassicsData : initMimicData
        ...
        console.warn('logicJson.value', logicJson.value)
  ...
  const headerStyle = computed(() => {
    return {
      ...
      zIndex: '2',
      height: 'auto',
      backgroundColor: isDark.value ? 'var(--el-bg-color)' : 'var(--el-bg-color)',
  ...
  async function saveJsonModel() {
    ...
    const jsonString = logicFlowJsonToWarmFlow(logicJson.value)
