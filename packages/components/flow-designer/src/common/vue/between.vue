...
<template>
  <div class="between">
    <EasyForm
      ref="formRef" class="betweenForm" :model="form" label-width="100px" :rules="rules" :disabled="disabled"
      label-position="left"
    >
      <EasyTabs v-model="tabsValue" type="line">
        ...
        <EasyTabPane label="监听器" name="2">
          <div class="listenerItem tabPane">
            <EasyTable
              :data="form.listenerRows" :columns="listenerColumns" :pagination="false" :show-index="false"
              style="width: 100%"
            >
        ...
        <EasyTabPane v-for="item in tabsList.slice(2)" :key="item.name" :label="item.label" :name="item.name">
          <div class="tabPane">
            <nodeExtList
              v-if="buttonList[item.name]?.length > 0" :ref="(el) => setNodeExtRef(el, item.name)"
