...
class SkipView extends CurvedEdge {
  getEdge(): h.JSX.Element {
    ...
    const points = this.pointFilter(pointsStr.split(' ').map(p => p.split(',').map(a => +a)))

    // 主路径
    const mainPath = getCurvedEdgePath(points, radius as number)
    ...
    // 跳转线是直线
    if (points.length === 2) {
      const nextEdge = model.graphModel.edges.filter(edge => edge.sourceNodeId === model.sourceNode.id)
    ...
    // 绘制主路径
    const mainPathElement: h.JSX.Element[] = []
...
`,
  ...
  private getAddElements(midPoint: number[], obj: any, isCondition: boolean = false) {
    ...
    const plusElements: h.JSX.Element[] = []
    plusElements.push(
      h(
        'g',
        {
