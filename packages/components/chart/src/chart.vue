...
<template>
  ...
  >
    ...
    <!-- 图例 -->
    <div
      v-if="showLegend && legendItems.length > 0" class="easy-chart__legend"
      :class="`easy-chart__legend--${legendPosition}`"
    >
      <div
        v-for="item in legendItems" :key="item.name" class="easy-chart__legend-item"
    ...
    <!-- SVG 绘图区 -->
    <div
      ref="bodyRef" class="easy-chart__body" :class="{ 'is-scrollable': scrollable, 'is-dragging': isDragging }"
      @wheel.prevent="onWheel"
    >
      <svg
        v-if="svgWidth > 0" :width="svgWidth" :height="svgHeight" class="easy-chart__svg" @mousemove="onMouseMove"
      ...
      >
        ...
        <template v-if="type === 'line' || type === 'bar' || type === 'stack' || type === 'mixed'">
          ...
          <g v-if="showGrid" class="easy-chart__grid">
            ...
            <template v-if="type === 'mixed'">
              <line
                v-for="(tick, i) in mixedBarYTicks" :key="`hy-${i}`" :x1="padding.left" :y1="getMixedBarY(tick)"
                :x2="svgWidth - padding.right" :y2="getMixedBarY(tick)" class="easy-chart__grid-line"
              />
            </template>
            <template v-else>
              <line
                v-for="(tick, i) in visibleYTicks" :key="`hy-${i}`" :x1="padding.left" :y1="getY(tick)"
          ...
          <g class="easy-chart__axis-y">
            <line
              :x1="padding.left" :y1="padding.top" :x2="padding.left" :y2="svgHeight - padding.bottom"
            ...
            <template v-if="type === 'mixed'">
              <g v-for="(tick, i) in mixedBarYTicks" :key="`myt-${i}`">
                <text
                  :x="padding.left - 8" :y="getMixedBarY(tick)" class="easy-chart__axis-text" text-anchor="end"
            ...
            <template v-else>
              <g v-for="(tick, i) in visibleYTicks" :key="`yt-${i}`">
                <text
                  :x="padding.left - 8" :y="getY(tick)" class="easy-chart__axis-text" text-anchor="end"
                  dominant-baseline="middle"
                >
                  {{ formatValue(tick) }}
                </text>
          ...
          <g v-if="type === 'mixed'" class="easy-chart__axis-y-right">
            <line
              :x1="svgWidth - padding.right" :y1="padding.top" :x2="svgWidth - padding.right"
              :y2="svgHeight - padding.bottom" class="easy-chart__axis-line"
            />
            <g v-for="(tick, i) in mixedLineYTicks" :key="`myrt-${i}`">
              <text
                :x="svgWidth - padding.right + 8" :y="getMixedLineY(tick)" class="easy-chart__axis-text"
          ...
          <g class="easy-chart__axis-x">
            <line
              :x1="padding.left" :y1="svgHeight - padding.bottom" :x2="svgWidth - padding.right"
              :y2="svgHeight - padding.bottom" class="easy-chart__axis-line"
            />
            <!-- X轴标签随 scroll 偏移，并裁剪 -->
            <g :clip-path="`url(#${clipPathId})`">
              <g v-for="(label, i) in xLabels" :key="`xl-${i}`">
                <text
                  :x="getXCenter(i)" :y="svgHeight - padding.bottom + 18" class="easy-chart__axis-text"
                  text-anchor="middle"
                >
                  {{ label }}
                </text>
          ...
          <template v-if="type === 'line'">
            <g :clip-path="`url(#${clipPathId})`">
              <g v-for="(serie, si) in visibleSeries" :key="`ls-${si}`">
                <!-- 面积填充 -->
                <path
                  v-if="serie.areaFill !== false && areaFill" :d="getAreaPath(serie.data)"
                  :fill="serie.color || defaultColors[si % defaultColors.length]" opacity="0.12"
                />
                <!-- 折线 -->
                <path
                  :d="getLinePath(serie.data)" :stroke="serie.color || defaultColors[si % defaultColors.length]"
                ...
                <template v-for="(val, i) in serie.data" :key="`lp-${i}`">
                  <circle
                    v-if="showDots" :cx="getXCenter(i)" :cy="getY(val)" :r="activeIndex === i ? 6 : 4"
                  ...
                  <template v-for="(v, idx) in serie.data" :key="`ll-${idx}`">
                    <text
                      v-if="showLabel" :x="getXCenter(idx)" :y="getY(v) - 10" text-anchor="middle"
          ...
          <template v-if="type === 'bar'">
            <g :clip-path="`url(#${clipPathId})`">
              <g v-for="(serie, si) in visibleSeries" :key="`bs-${si}`">
                <rect
                  v-for="(val, i) in serie.data" :key="`br-${i}`" :x="getBarX(i, si)" :y="getY(val)"
                ...
                <template v-for="(val, i) in serie.data" :key="`bl-${i}`">
                  <text
                    v-if="showLabel" :x="getBarX(i, si) + barWidth / 2" :y="getY(val) - 5" text-anchor="middle"
            ...
            <g :clip-path="`url(#${clipPathId})`">
              <g v-for="(serie, si) in visibleSeries" :key="`ss-${si}`">
                <template v-for="(seg, i) in stackSegments[si]" :key="`sr-${i}`">
                  <rect
                    v-if="seg.h > 0" :x="getStackBarX(i)" :y="seg.y" :width="stackBarWidth" :height="seg.h"
                  ...
                  <text
                    v-if="showLabel && seg.h > 16" :x="getStackBarX(i) + stackBarWidth / 2" :y="seg.y + seg.h / 2"
          ...
          <template v-if="type === 'mixed'">
            <g :clip-path="`url(#${clipPathId})`">
              ...
              <g v-for="(serie, si) in mixedBarSeries" :key="`mbs-${si}`">
                <rect
                  v-for="(val, i) in serie.data" :key="`mbr-${i}`" :x="getMixedBarX(i, serie._barIdx)"
                ...
                <template v-for="(val, i) in serie.data" :key="`mbl-${i}`">
                  <text
                    v-if="showLabel" :x="getMixedBarX(i, serie._barIdx) + mixedBarWidth / 2"
              ...
              <g v-for="(serie, si) in mixedLineSeries" :key="`mls-${si}`">
                <!-- 面积填充 -->
                <path
                  v-if="serie.areaFill !== false && areaFill" :d="getMixedLineAreaPath(serie.data)"
                  :fill="getSerieColor(serie, serie._origIdx)" opacity="0.12"
                />
                <!-- 折线 -->
                <path
                  :d="getMixedLinePath(serie.data)" :stroke="getSerieColor(serie, serie._origIdx)"
                  stroke-width="2.5" fill="none" stroke-linecap="round" stroke-linejoin="round" class="easy-chart__line"
                />
                <!-- 数据点 -->
                <template v-for="(val, i) in serie.data" :key="`mlp-${i}`">
                  <circle
                    v-if="showDots" :cx="getXCenter(i)" :cy="getMixedLineY(val)" :r="activeIndex === i ? 6 : 4"
                  ...
                  <template v-for="(v, idx) in serie.data" :key="`mll-${idx}`">
                    <text
                      v-if="showLabel" :x="getXCenter(idx)" :y="getMixedLineY(v) - 10" text-anchor="middle"
          ...
          <g v-if="scrollable" class="easy-chart__scrollbar">
            <!-- 轨道 -->
            <rect
              :x="padding.left" :y="svgHeight - 6" :width="plotWidth" height="4" rx="2"
              class="easy-chart__scrollbar-track"
            />
            <!-- 滑块 -->
            <rect
              :x="scrollbarThumbLeft" :y="svgHeight - 7" :width="scrollbarThumbW" height="6" rx="3"
        ...
        <template v-if="type === 'pie' || type === 'donut'">
          <g :transform="`translate(${pieCenter.x}, ${pieCenter.y})`">
            <path
              v-for="(slice, i) in pieSlices" :key="`ps-${i}`" :d="slice.path" :fill="slice.color"
            ...
            <template v-if="showLabel">
              <text
                v-for="(slice, i) in pieSlices" v-show="slice.percent >= 5" :key="`pl-${i}`" :x="slice.labelX"
            ...
            <template v-if="type === 'donut'">
              ...
              <text
                v-if="donutValue" text-anchor="middle" dominant-baseline="middle" y="18"
        ...
        <template v-if="type === 'funnel'">
          <g>
            <path
              v-for="(item, i) in funnelItems" :key="`fi-${i}`" :d="item.path" :fill="item.color"
            ...
            <g v-for="(item, i) in funnelItems" :key="`fl-${i}`">
              <text
                :x="item.labelX" :y="item.labelY - 8" text-anchor="middle" dominant-baseline="middle"
                class="easy-chart__funnel-label"
              >
                {{ item.name }}
              </text>
              <text
                :x="item.labelX" :y="item.labelY + 10" text-anchor="middle" dominant-baseline="middle"
        ...
        <template v-if="type === 'hbar'">
          ...
          <g v-if="showGrid">
            <line
              v-for="(tick, i) in hbarXTicks" :key="`hg-${i}`" :x1="getHBarTickX(tick)" :y1="padding.top"
          ...
          <g>
            ...
            />
            <text
              v-for="(tick, i) in hbarXTicks" :key="`hxt-${i}`" :x="getHBarTickX(tick)"
          ...
          <g>
            ...
            />
            <text
              v-for="(label, di) in labels ?? []" :key="`hyl-${di}`" :x="padding.left - 8"
          ...
          <g v-for="item in hbarItems" :key="`hb-${item.serieIdx}-${item.dataIdx}`">
            <rect
              :x="item.barX" :y="item.barY" :width="item.barW" :height="item.barH" :fill="item.color"
            ...
            <!-- 数值标签 -->
            <text
              v-if="item.barW > 20" :x="item.valX" :y="item.barY + item.barH / 2" class="easy-chart__hbar-val"
              dominant-baseline="middle"
            >
          ...
          <!-- hover 高亮行 -->
          <rect
            v-if="activeHBarIndex >= 0" :x="padding.left" :y="padding.top + hbarRowH * activeHBarIndex"
        ...
        <template v-if="type === 'gauge'">
          ...
          <!-- 轨道分段弧（颜色区段） -->
          <path
            v-for="(arc, i) in gaugeArcs" :key="`ga-${i}`" :d="arc.path" :stroke="arc.color"
          ...
          <!-- 刻度文字（靠近轨道外圈，不画线） -->
          <text
            v-for="(tick, i) in gaugeTicks" :key="`gtick-${i}`" :x="tick.lx" :y="tick.ly"
