...
<template>
  <div class="tree-node-wrapper" :class="`layout-${layout}`">
    ...
    >
      ...
      <span v-if="isLoading" class="tree-node__loading">
        <svg class="tree-node__loading-icon" width="16" height="16" viewBox="0 0 24 24">
          <circle
            cx="12" cy="12" r="10" stroke="currentColor" stroke-width="2" fill="none" stroke-dasharray="32"
      ...
      <!-- 展开收起按钮 -->
      <button
        v-else-if="(hasChildren || hasRemoteChildren) && expandable" class="tree-node__arrow"
      ...
      >
        ...
        <svg v-if="layout === 'vertical' && !isExpanded" viewBox="0 0 24 24" width="16" height="16">
          <path
            d="M6 9l6 6 6-6" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round"
            fill="none"
          />
        </svg>
        <svg v-else-if="layout === 'vertical' && isExpanded" viewBox="0 0 24 24" width="16" height="16">
          <path
            d="M18 15l-6-6-6 6" stroke="currentColor" stroke-width="2.5" stroke-linecap="round"
        ...
        <svg v-else-if="layout === 'horizontal' && !isExpanded" viewBox="0 0 24 24" width="16" height="16">
          <path
            d="M15 18l-6-6 6-6" stroke="currentColor" stroke-width="2.5" stroke-linecap="round"
            stroke-linejoin="round" fill="none"
          />
        </svg>
        <svg v-else viewBox="0 0 24 24" width="16" height="16">
          <path
            d="M9 18l6-6-6-6" stroke="currentColor" stroke-width="2.5" stroke-linecap="round"
            stroke-linejoin="round" fill="none"
          />
        </svg>
      </button>
    ...
    <div v-if="hasChildren && isExpanded" class="tree-children">
      <TreeNode
        v-for="(child, index) in children" :key="child[nodeConfig.keyField as string] ?? index" :node="child"
