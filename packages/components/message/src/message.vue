...
<template>
  <Teleport to="body">
    <template v-for="pos in positions" :key="pos">
      <TransitionGroup
        v-if="groupedList[pos]?.length" name="easy-message" tag="div" class="easy-message-container"
        :class="`easy-message-container--${pos}`"
      >
        <div
          v-for="item in groupedList[pos]" :key="item.id" class="easy-message"
        ...
        >
          ...
          <div v-if="item.type !== 'text'" class="easy-message__icon">
            <svg
              v-if="item.type === 'success'" viewBox="0 0 24 24" width="1em" height="1em" fill="none"
            ...
            <svg
              v-else-if="item.type === 'warning'" viewBox="0 0 24 24" width="1em" height="1em" fill="none"
              stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"
            >
            ...
            <svg
              v-else-if="item.type === 'danger'" viewBox="0 0 24 24" width="1em" height="1em" fill="none"
              stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"
            >
              <circle cx="12" cy="12" r="10" />
            ...
            <svg
              v-else-if="item.type === 'info'" viewBox="0 0 24 24" width="1em" height="1em" fill="none"
              stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"
            >
              <circle cx="12" cy="12" r="10" />
          ...
          <div v-if="item.showProgress" class="easy-message__progress">
            <div
              class="easy-message__progress-bar" :class="`easy-message__progress-bar--${item.type}`"
