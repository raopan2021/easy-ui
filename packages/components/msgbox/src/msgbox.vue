...
<template>
  <Teleport to="body">
    <Transition name="easy-msgbox-fade">
      <div v-if="msgboxState.visible" class="easy-msgbox-overlay easy-msgbox-overlay--center"
        @click.self="handleOverlayClick">
        <Transition name="easy-msgbox-zoom">
          <div v-if="msgboxState.visible" class="easy-msgbox" :class="[msgboxState.options.customClass]" role="dialog"
            aria-modal="true" @keydown.esc="handleEsc">
            <!-- 关闭按钮 -->
            <button v-if="msgboxState.options.showClose !== false" class="easy-msgbox__close" aria-label="关闭"
              @click="handleClose">
            ...
            <div class="easy-msgbox__header" :class="{ 'easy-msgbox__header--no-title': !msgboxState.options.title }">
              <div v-if="showIcon" class="easy-msgbox__icon"
                :class="`easy-msgbox__icon--${msgboxState.options.type || 'info'}`">
                <!-- success -->
                <svg v-if="msgboxState.options.type === 'success'" viewBox="0 0 24 24" fill="none" stroke="currentColor"
                  stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
                  <circle cx="12" cy="12" r="10" />
                  <polyline points="8,12 11,15 16,9" />
                </svg>
                <!-- warning -->
                <svg v-else-if="msgboxState.options.type === 'warning'" viewBox="0 0 24 24" fill="none"
                ...
                <!-- danger -->
                <svg v-else-if="msgboxState.options.type === 'danger'" viewBox="0 0 24 24" fill="none"
                  stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
                  <circle cx="12" cy="12" r="10" />
            ...
            <div class="easy-msgbox__body">
              ...
              <p v-if="msgboxState.options.dangerouslyUseHTMLString" class="easy-msgbox__message"
              ...
              <div v-if="msgboxState.options.showInput" class="easy-msgbox__input-wrap">
                <textarea v-if="msgboxState.options.input?.inputType === 'textarea'" ref="inputRef"
                ...
                <input v-else ref="inputRef" v-model="msgboxState.inputValue" class="easy-msgbox__input"
                  :class="{ 'easy-msgbox__input--error': msgboxState.inputError }"
            ...
            <div class="easy-msgbox__footer">
              <button v-if="msgboxState.options.showCancelButton !== false && msgboxState.options.showCancelButton"
