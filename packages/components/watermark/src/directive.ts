...
function renderWatermark(canvas: HTMLCanvasElement, container: HTMLElement, options: WatermarkOptions) {
  const {
    content = '',
    image = '',
    rotate = -22,
...
function applyTileToOverlay(tileCanvas: HTMLCanvasElement, overlay: HTMLCanvasElement, container: HTMLElement) {
  const ratio = window.devicePixelRatio || 1
