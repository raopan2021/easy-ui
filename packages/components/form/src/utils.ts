 ...
 * 用法：
 * ```ts
 * import { required, email, minLength, custom } from './utils'
 *
 * // 方法一：传统函数式写法（保留）
 * const rules1 = {
...
export function email(message = '请输入正确的邮箱地址'): FormRule {
  return {
    validator: (v) => {
      if (!v)
        return true
      return /^[^\s@]+@[^\s@][^\s.@]*\.[^\s@]+$/.test(String(v)) || message
    },
  }
}
...
export function numRange(options: RangeOptions): FormRule {
  const { min, max, minInclusive = true, maxInclusive = true, minMessage, maxMessage, message } = options
  ...
  return {
    validator: (v) => {
      ...
      if (min !== undefined) {
        ...
        if (!valid)
          return minMessage || (minInclusive ? `不能小于 ${min}` : `必须大于 ${min}`)
      }
      if (max !== undefined) {
        const valid = maxInclusive ? num <= max : num < max
        if (!valid)
          return maxMessage || (maxInclusive ? `不能超过 ${max}` : `必须小于 ${max}`)
      }
      return true
    },
    trigger: 'change',
...
export function custom(
  fn: (value: any, formData?: Record<string, any>) => string | true | Promise<string | true>,
 ...
 */
export async function validateField(value: any, rules: Rule[], formData?: Record<string, any>): Promise<string | null> {
  // 规范化规则
