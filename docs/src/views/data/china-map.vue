...
<template>
  <div class="china-map-doc">
    ...
    <div class="doc-header">
      ...
      <p class="doc-desc">
        基于 SVG 原生实现的中国地图可视化组件，支持省份数据着色、气泡展示、Tooltip
        悬浮、点击事件等功能，零依赖、开箱即用。
      </p>
    </div>
    ...
    <section class="doc-section">
      ...
      <div class="doc-preview">
        <div class="doc-preview__body">
          <EasyChinaMap
            title="2025年各省份 GDP 总量" subtitle="单位：亿元" :data="gdpData" :height="680" value-label="GDP"
...
/>"
    ...
    <section class="doc-section">
      ...
      <h3 class="doc-section__subtitle">
        对比模式（tooltip-mode="compare"）
      </h3>
      <div class="doc-preview">
        <div class="doc-preview__body">
          <EasyChinaMap
            title="省份 GDP 对比" subtitle="单位：亿元 | Tooltip 同时展示所有年份数据" :data="multiYearData" :height="420"
...
/>"
      ...
      <!-- 切换模式 -->
      <h3 class="doc-section__subtitle">
        切换模式（tooltip-mode="switch"）
      </h3>
      <div class="doc-preview">
        <div class="doc-preview__body">
          <EasyChinaMap
            title="省份 GDP 切换" subtitle="单位：亿元 | 点击 Tab 切换数据集" :data="multiYearData" :height="420"
...
/>"
  ...
  },
  {
    ...
    <section class="doc-section">
      ...
      <p class="doc-section__desc">
        通过 <code>label-mode</code> 属性切换省份名称显示模式：<code>short</code> 显示简称（如"粤"），<code>full</code>
        显示完整名称（如"广东省"）。 简称更适合小尺寸地图，完整名称更适合大尺寸或需要明确标识的场景。
      </p>
      <div class="doc-preview">
        <div class="doc-preview__body">
          <div class="mode-switch">
            ...
            <button
              class="mode-switch__btn" :class="{ 'is-active': labelMode === 'short' }"
          ...
          <EasyChinaMap
            :title="labelMode === 'short' ? '简称模式 — 简洁清晰' : '全称模式 — 信息完整'" :data="gdpData" :height="420"
...
/>"
    ...
    <section class="doc-section">
      ...
      <p class="doc-section__desc">
        通过 <code>color-range</code> 属性自定义颜色渐变区间 <code>[最小色, 最大色]</code>， 通过
        <code>empty-color</code> 设置无数据省份的填充色。
      </p>
      <div class="doc-preview">
        <div class="doc-preview__body">
          <div class="map-row">
            <div class="map-item">
              <EasyChinaMap
                title="绿色系 — 用户覆盖率" :data="coverageData" :height="320" :color-range="['#d1fae5', '#065f46']"
                empty-color="#f1f5f9" value-label="覆盖率" legend-title="覆盖率（%）"
              />
            </div>
            <div class="map-item">
              <EasyChinaMap
                title="橙红系 — 投诉数量" :data="complaintData" :height="320" :color-range="['#fef3c7', '#b45309']"
...
/>"
    ...
    <section class="doc-section">
      ...
      <p class="doc-section__desc">
        设置 <code>show-bubble</code> 可在各省份中心叠加气泡圆圈，气泡大小按数值比例缩放，适合展示点数据分布。 通过
        <code>bubble-color</code> 自定义气泡颜色。
      </p>
      <div class="doc-preview">
        <div class="doc-preview__body">
          <EasyChinaMap
            title="各省份活跃用户分布" subtitle="气泡大小代表活跃用户量" :data="userCountData" :height="460" :show-bubble="true"
...
/>"
    ...
    <section class="doc-section">
      ...
      <div class="doc-preview">
        <div class="doc-preview__body">
          <EasyChinaMap
            title="极简风格" :data="gdpData" :height="400" :show-label="false"
...
/>"
    ...
    <section class="doc-section">
      ...
      <p class="doc-section__desc">
        地图默认支持滚轮缩放和鼠标拖拽平移，通过左下角的缩放控制按钮可精确调整。 设置
        <code>:zoomable="false"</code> 可禁用缩放功能。
      </p>
      <div class="doc-preview">
        <div class="doc-preview__body">
          <EasyChinaMap
            title="支持缩放平移" subtitle="滚轮缩放 / 拖拽平移 / 左下角按钮" :data="gdpData" :height="420" value-label="GDP"
            legend-title="GDP（亿元）" @click="handleClick"
          />
        </div>
        <EasyDocCode
...
/>"
    ...
    <section class="doc-section">
      ...
      <div class="doc-preview">
        <div class="doc-preview__body">
          <EasyChinaMap
            title="全国地图底图" :data="[]" :height="380" empty-color="#dbeafe" stroke-color="#93c5fd"
            :show-legend="false"
          />
        </div>
        <EasyDocCode
      ...
      <div class="doc-preview">
        <table class="doc-table">
          ...
          <tbody>
            ...
            <tr>
              <td>label-mode</td>
              <td>标签模式：short 显示简称，full 显示完整名称</td>
              <td>'short' / 'full'</td>
              <td>'short'</td>
            </tr>
            <tr>
            ...
            <tr>
              <td>tooltip-mode</td>
              <td>Tooltip 展示模式：compare 同时展示所有数据，switch Tab 切换模式</td>
              <td>'compare' / 'switch'</td>
              <td>'compare'</td>
            </tr>
          </tbody>
        </table>
