...
const structureData: TreeChartNode[] = [
  {
    ...
    children: [
      ...
      },
      {
        id: 'child-3',
        label: '子节点 3',
        children: [{ id: 'leaf-3', label: '叶子节点 C' }],
      },
    ],
  },
]
...
function onToggleExpand(node: TreeChartNode, expanded: boolean) {
  ...
  expandLog.value = JSON.stringify(
    { id: node.id, label: node.label, expanded, controlledKeys: controlledKeys.value },
    null,
    2,
  )
  setTimeout(() => {
...
/>"
    ...
    <section class="doc-section">
      ...
      <p class="doc-section__desc">
        通过 <code>width</code> 和
        <code>height</code> 自定义画布尺寸，支持数字（像素）或字符串（百分比）。不设置时默认充满父容器。
      </p>
      <div class="doc-preview">
        <div class="doc-preview__body">
